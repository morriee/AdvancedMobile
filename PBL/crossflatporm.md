...
#### 단점 
      ...
      3. 외부 라이브러리 (google,naver) 등등 을 제공하지 않는다.
      4. 앱이 복잡해진다면 속도가 느리다.
      5. 복잡한 UI구성이 힘들다.
...
<h4>단점</h4>
1. Dart
구글에서 개발한 언어. 자바와 c#을 알면 쉽게 학습이 가능하지만 python이나 javascript에서 자유로운 구조를 사용했다면 학습에 조금 답답할수 있음. 
많이 사용되지 않는 언어임.
평가가 좋지 않은 언어이기도 함.

2. 플러그인
플러그인이 부족함. 어플 생성시 웹뷰, 맵등 플러그인이 필요하나 1.0을 넘는 버전을 보기가 힘듬편.
계속해서 업데이트 되는 버전들을 다시 붙이고 테스트 해보는 것은 굉장히 번거로운 일이 아닐 수 없음.

3. eco
플러터 레퍼런스 찾기가 굉장히 힘듬.
공식문서도 있고, Flutter Codelab등을 지원하여 이를 통해 학습이 가능하지만 어플을 개발해가면서 원하는 내용을 검색해봐도 알맞은 결과를 얻기가 쉽지않음. 특히 원하는 내용을 한글로된 블로그에서 찾기는 하늘에 별따기.
### 3) Cordova 
